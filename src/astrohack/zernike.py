...
class ZernikeFitter():
    ...
    def fit_complex_aperture(self, antenna, ddi='ddi_0'):
        ...
        aperture_amp = image_mds[antenna][ddi]['AMPLITUDE']
        aperture_phase = image_mds[antenna][ddi]['CORRECTED_PHASE']
        ...
        maskidx = np.where(np.hypot(xx, yy) > 1)
        ...
        coeffsreal = leastsq(self.fit_minimize,np.ones(self.ntermsreal),
                    args=(aperture_real, xx, yy, maskidx))[0]
        coeffsimag = leastsq(self.fit_minimize,np.ones(self.ntermsimag),
                    args=(aperture_imag, xx, yy, maskidx))[0]
